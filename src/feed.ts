...
export class FeedClient {
  ...
  async getPost(entityKey: string): Promise<SocialPost | null> {
    const result = await this.cdn.entity
      .query()
      .where([eq(ATTR_TYPE, SOCIAL_POST_TYPE)])
      .withPayload(true)
      .fetch()
  ...
  async getUserPosts(
    ...
    const result = await this.cdn.entity
      .query()
      .where([eq(ATTR_TYPE, SOCIAL_POST_TYPE), eq(ATTR_UUID, uuid)])
      .withPayload(true)
      .fetch()
  ...
  async getReactions(
    ...
    const result = await this.cdn.entity
      .query()
      .where([eq(ATTR_TYPE, SOCIAL_REACTION_TYPE), eq(ATTR_TARGET_KEY, targetEntityKey)])
      .withPayload(true)
      .fetch()
  ...
  async getComments(
    ...
    const result = await this.cdn.entity
      .query()
      .where([eq(ATTR_TYPE, SOCIAL_COMMENT_TYPE), eq(ATTR_TARGET_KEY, targetEntityKey)])
      .withPayload(true)
      .fetch()
  ...
  private async _getCommentsByKey(entityKey: string): Promise<SocialComment[]> {
    const result = await this.cdn.entity
      .query()
      .where([eq(ATTR_TYPE, SOCIAL_COMMENT_TYPE)])
      .withPayload(true)
      .fetch()
    return result.entities
