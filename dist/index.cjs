...
var SocialClient = class SocialClient {
	...
	*/
	async follow(targetUuid) {
		...
		const { entityKey } = await this.cdn.entity.create({
			payload: (0, arka_cdn.jsonToPayload)(follow),
			contentType: "application/json",
			attributes: [
				{
					key: ATTR_TYPE,
	...
	*/
	async getFollowing(options = {}) {
		...
		return applyPagination$1((await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FOLLOW_TYPE), (0, arka_cdn.eq)(ATTR_UUID, uuid)]).withPayload(true).fetch()).entities.map((e) => e.toJson()).filter((f) => f.status === "active"), offset, limit);
	}
	/**
	...
	*/
	async getFollowers(options = {}) {
		const { uuid = this.uuid, limit, offset = 0 } = options;
		return applyPagination$1((await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FOLLOW_TYPE), (0, arka_cdn.eq)(ATTR_TARGET_UUID, uuid)]).withPayload(true).fetch()).entities.map((e) => e.toJson()).filter((f) => f.status === "active"), offset, limit);
	}
	/**
	...
	*/
	async sendFriendRequest(targetUuid, message) {
		...
		const { entityKey } = await this.cdn.entity.create({
			payload: (0, arka_cdn.jsonToPayload)(request),
			contentType: "application/json",
			attributes: [
				{
					key: ATTR_TYPE,
	...
	*/
	async _respondToRequest(entityKey, status) {
		const entity = (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FRIEND_REQUEST_TYPE), (0, arka_cdn.eq)(ATTR_TARGET_UUID, this.uuid)]).withPayload(true).fetch()).entities.find((e) => e.key === entityKey);
	...
	*/
	async cancelFriendRequest(entityKey) {
		const entity = (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FRIEND_REQUEST_TYPE), (0, arka_cdn.eq)(ATTR_UUID, this.uuid)]).withPayload(true).fetch()).entities.find((e) => e.key === entityKey);
		if (!entity) throw new Error(`ASide: friend request "${entityKey}" not found`);
		const req = entity.toJson();
		const updated = {
			...req,
	...
	*/
	async getIncomingFriendRequests() {
		return (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FRIEND_REQUEST_TYPE), (0, arka_cdn.eq)(ATTR_TARGET_UUID, this.uuid)]).withPayload(true).fetch()).entities.map((e) => e.toJson()).filter((r) => r.status === "pending");
	}
	/**
	* Returns pending friend requests sent by the current user.
	*/
	async getOutgoingFriendRequests() {
		return (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FRIEND_REQUEST_TYPE), (0, arka_cdn.eq)(ATTR_UUID, this.uuid)]).withPayload(true).fetch()).entities.map((e) => e.toJson()).filter((r) => r.status === "pending");
	}
	/**
	...
	*/
	async getFriends(options = {}) {
		...
		return applyPagination$1((await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FRIEND_REQUEST_TYPE)]).withPayload(true).fetch()).entities.map((e) => e.toJson()).filter((r) => r.status === "accepted" && (r.fromUuid === this.uuid || r.toUuid === this.uuid)), offset, limit);
	}
	/**
	...
	*/
	async block(targetUuid) {
		...
		const { entityKey } = await this.cdn.entity.create({
			payload: (0, arka_cdn.jsonToPayload)(blockData),
			contentType: "application/json",
			attributes: [
				{
					key: ATTR_TYPE,
	...
	*/
	async unblock(targetUuid) {
		...
		await this.cdn.entity.update({
			entityKey: existing.entityKey,
			payload: (0, arka_cdn.jsonToPayload)({
				...existing,
				status: "removed"
			}),
			contentType: "application/json",
	...
	*/
	async getBlockedUsers(options = {}) {
		const { limit, offset = 0 } = options;
		return applyPagination$1((await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_BLOCK_TYPE), (0, arka_cdn.eq)(ATTR_UUID, this.uuid)]).withPayload(true).fetch()).entities.map((e) => e.toJson()).filter((b) => b.status === "active"), offset, limit);
	}
	async _findFollow(followerUuid, followeeUuid) {
		const entity = (await this.cdn.entity.query().where([
			(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_FOLLOW_TYPE),
			(0, arka_cdn.eq)(ATTR_UUID, followerUuid),
			(0, arka_cdn.eq)(ATTR_TARGET_UUID, followeeUuid)
	...
	async _findBlock(byUuid, blockedUuid) {
		const entity = (await this.cdn.entity.query().where([
			(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_BLOCK_TYPE),
			(0, arka_cdn.eq)(ATTR_UUID, byUuid),
			(0, arka_cdn.eq)(ATTR_TARGET_UUID, blockedUuid)
		]).withPayload(true).fetch()).entities[0];
		if (!entity) return null;
		return {
			...entity.toJson(),
...
var FeedClient = class {
	...
	*/
	async createPost(options) {
		...
		const { entityKey } = await this.cdn.entity.create({
			payload: (0, arka_cdn.jsonToPayload)(post),
			contentType: "application/json",
			attributes: [
				{
					key: ATTR_TYPE,
	...
	*/
	async getPost(entityKey) {
		const entity = (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_POST_TYPE)]).withPayload(true).fetch()).entities.find((e) => e.key === entityKey);
		if (!entity) return null;
	...
	*/
	async getUserPosts(options = {}) {
		const { uuid = this.uuid, limit, offset = 0 } = options;
		return applyPagination((await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_POST_TYPE), (0, arka_cdn.eq)(ATTR_UUID, uuid)]).withPayload(true).fetch()).entities.map((e) => ({
			...e.toJson(),
			entityKey: e.key
	...
	*/
	async react(targetEntityKey, type = "like") {
		...
		const { entityKey } = await this.cdn.entity.create({
			payload: (0, arka_cdn.jsonToPayload)(reaction),
			contentType: "application/json",
			attributes: [
				{
					key: ATTR_TYPE,
	...
	*/
	async getReactions(targetEntityKey, type) {
		return (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_REACTION_TYPE), (0, arka_cdn.eq)(ATTR_TARGET_KEY, targetEntityKey)]).withPayload(true).fetch()).entities.map((e) => ({
			...e.toJson(),
			entityKey: e.key
	...
	*/
	async addComment(targetEntityKey, content) {
		...
		const { entityKey } = await this.cdn.entity.create({
			payload: (0, arka_cdn.jsonToPayload)(comment),
			contentType: "application/json",
			attributes: [
				{
					key: ATTR_TYPE,
	...
	*/
	async getComments(targetEntityKey, options = {}) {
		const { limit, offset = 0 } = options;
		return applyPagination((await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_COMMENT_TYPE), (0, arka_cdn.eq)(ATTR_TARGET_KEY, targetEntityKey)]).withPayload(true).fetch()).entities.map((e) => ({
			...e.toJson(),
			entityKey: e.key
	...
	async _findReaction(targetEntityKey, reactorUuid, type) {
		const entity = (await this.cdn.entity.query().where([
			(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_REACTION_TYPE),
			(0, arka_cdn.eq)(ATTR_UUID, reactorUuid),
			(0, arka_cdn.eq)(ATTR_TARGET_KEY, targetEntityKey)
	...
	async _getCommentsByKey(entityKey) {
		return (await this.cdn.entity.query().where([(0, arka_cdn.eq)(ATTR_TYPE, SOCIAL_COMMENT_TYPE)]).withPayload(true).fetch()).entities.filter((e) => e.key === entityKey).map((e) => ({
			...e.toJson(),
			entityKey: e.key
		}));
	}
...
exports.ATTR_TARGET_KEY = ATTR_TARGET_KEY;
exports.ATTR_TARGET_UUID = ATTR_TARGET_UUID;
...
exports.ProfileWatcher = ProfileWatcher;
exports.SNOWFLAKE_EPOCH = SNOWFLAKE_EPOCH;
exports.SOCIAL_BLOCK_TYPE = SOCIAL_BLOCK_TYPE;
exports.SOCIAL_COMMENT_TYPE = SOCIAL_COMMENT_TYPE;
exports.SOCIAL_FOLLOW_TYPE = SOCIAL_FOLLOW_TYPE;
exports.SOCIAL_FRIEND_REQUEST_TYPE = SOCIAL_FRIEND_REQUEST_TYPE;
exports.SOCIAL_POST_TYPE = SOCIAL_POST_TYPE;
exports.SOCIAL_REACTION_TYPE = SOCIAL_REACTION_TYPE;
exports.SnowflakeGenerator = SnowflakeGenerator;
exports.SocialClient = SocialClient;
exports.aesDecrypt = aesDecrypt;
exports.aesEncrypt = aesEncrypt;
exports.decodeFriendRequest = decodeFriendRequest;
exports.decodeProfileLink = decodeProfileLink;
exports.ecdhDeriveKeys = ecdhDeriveKeys;
exports.encodeFriendRequest = encodeFriendRequest;
exports.encodeProfileLink = encodeProfileLink;
exports.friendRequestQRExpiresIn = friendRequestQRExpiresIn;
exports.generateAesKey = generateAesKey;
exports.generateAppKeyPair = generateAppKeyPair;
exports.hmacSign = hmacSign;
exports.hmacVerify = hmacVerify;
exports.isFriendRequestQRValid = isFriendRequestQRValid;
exports.parseAsideUri = parseAsideUri;
exports.phraseToCommitment = phraseToCommitment;
exports.verifyPhraseCommitment = verifyPhraseCommitment;
